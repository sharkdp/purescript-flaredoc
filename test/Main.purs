...
import Test.FlareDoc (class Interactive, document, parseModuleJSON)

newtype UnconsRecord = UnconsRecord { head :: Int, tail :: Array Int }

derive instance genericUnconsRecord :: Generic UnconsRecord

main :: Eff (chan :: Chan, dom :: DOM, err :: EXCEPTION, ajax :: AJAX, console :: CONSOLE) Unit
main = launchAff do
  r <- get "purescript-arrays.json"
  let dictionary = parseModuleJSON r.response
  case dictionary of
    Left err -> do
      liftEff $ (log err :: Eff _ Unit)
      return unit
    Right dict -> do
      let doc :: forall t. Interactive t => _ -> t -> _
          doc = document dict "Data.Array"
      liftEff do
        doc "range" range
